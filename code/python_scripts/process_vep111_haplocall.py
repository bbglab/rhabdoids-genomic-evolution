...
def get_mane_transcript(group):
    """
    For each variant in each gene, that is unique combination of #CHROM, POS, REF, ALT, and GENE it keeps the MANE transcript.
    In the case of not having MANE, it will select CANONICAL transcript annotations.
    ...
    :param group: groupby object (~df slice) of each variant and all the reported annotations by VEP111
    :return:a one row data frame for each variant
    """
    name, dff = group
    if len(dff) > 1:
        if True in dff['IS_MANE']:
            dff = dff[dff['MANE_SELECT'] != '-']
            if len(dff) > 1:
                ...
                for cons in DAMAGING_CONSEQ:
                    if cons in conseqs:
                        ...
                        if len(dff) > 1:
                            dff = dff.drop_duplicates(subset=['#Uploaded_variation','Gene'], keep='first')
                            break
                        else:
                            break
            else:
...
def separate(rw):
    ...
    if "_" in rw['#Uploaded_variation']:
        ...
        if "-" in rw['POS']:
            ...
            rw['POS'] = int(rw['POS'])
        rw['REF'] = hg38(str(rw['#CHROM']), rw['POS'], 1)
        rw['ALT'] = rw['Allele']
    return rw
...
@click.option('--path_input_vep',
              ...
              required = True,
              help="Path to the folder that contains the tab delimited files of the maf annotated by vep111"
                     "(one file per chromosome). E.g. 'AQ5174_chr1_vep.tsv'")
@click.option('--path_input_maf',
              '-im',
              required = True,
              help="Path to the folder that contains the tab delimited files of the original mafs"
                     "(one file per chromosome). E.g. 'AQ5174_chr1.maf.gz'")
@click.option('--cores',
              '-c',
              required = True,
...
def cli(path_input_vep, path_input_maf,cores):
    """
    ...
    df_ann = read_vcf(path_input_vep, sep='\t')
    
    # read original maf not annotated
    df_maf = pd.read_csv(path_input_maf, sep='\t')

    # get one annotation per variant
    df_ann_filt = pd.DataFrame()
    
    df_ann['MANE_SELECT'] = df_ann['MANE_SELECT'].fillna('-')
    df_ann['IS_MANE'] = df_ann['MANE_SELECT'].apply(lambda x: True if 'NM' in x else False)

    with Pool(int(cores)) as pool:
        for df_res in tqdm(pool.imap(get_mane_transcript, df_ann.groupby(["#Uploaded_variation","Gene"])),
                           total=len(df_ann.groupby(["#Uploaded_variation","Gene"]))):
            df_ann_filt = pd.concat([df_ann_filt,df_res], ignore_index=True)
    ...
    if 'SAMPLE' not in df_maf.columns:
        df_maf['SAMPLE'] = path_input_maf.split('/')[-1].split('_')[0]
    ...
    df_maf = df_maf[['#CHROM', 'POS', 'ID', 'REF', 'ALT', 'QUAL', 'FILTER', 'INFO', 'FORMAT', 'NORMAL',
                     ...
                     'IMPACT','DISTANCE', 'STRAND', 'FLAGS', 'SYMBOL', 'SYMBOL_SOURCE', 'HGNC_ID', 'CANONICAL','MANE_SELECT', 'ENSP',
    ...
    out_file = out_file.replace('_vep.tsv','.maf.gz')
