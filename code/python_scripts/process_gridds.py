...
file = '/data/bbg/projects/sjd_pediatric_tumors/data/hg38.chrom.sizes'
with open(file) as f:
    rows = (line.split('\n')[0] for line in f)
...
def gene_info_annotations(pt1_df):
    ...
    drivers_df = pd.read_csv('/data/bbg/datasets/intogen/output/runs/v2023/20230224_release2023/run/intogen_analysis/unique_drivers.tsv',sep='\t')
    ...
    germ_old_df = pd.read_excel('/data/bbg/projects/sjd_pediatric_tumors/data/germline_muts_data/41586_2014_BFnature12981_MOESM96_ESM.xlsx')
    ...
    germ_new_df = pd.read_excel('/data/bbg/projects/sjd_pediatric_tumors/data/germline_muts_data/43018_2021_172_MOESM2_ESM.xlsx',sheet_name='Supp Table 3',skiprows=1)
    ...
    role_df = pd.read_csv('/data/bbg/projects/sjd_pediatric_tumors/data/consensus_role_cancer_genes.tsv',sep='\t')
    ...
    cgc_transl_df = pd.read_csv('/data/bbg/projects/sjd_pediatric_tumors/data/Census_transFri_Apr_9_10_22_06_2021.tsv',sep='\t')
...
def annotate_sv_genes (sv_df,pos_df):
    ...
    if len(sv_pos_df) == 0:
        sv_final_df = pd.DataFrame(columns= ['SYMBOL germline','germline_mskcc','intogen role','mut','fusion','cgc_transl','chr/chr','sv_type','distance','distance_rel'])
    else:
...
@click.option('--canonical_transcripts_file',
              ...
              required=False,
              default = None,
              help="path to file with the annotation of the canonical transcripts from ensembl")
@click.option('--mane_transcripts_file',
              '-mt',
              required=False,
              default = None,
              help="path to file with the annotation of the MANE transcripts")
...
def cli(input_vcf, output_dir, genomic_positions_file, canonical_transcripts_file, mane_transcripts_file, tumor_sample_id, normal_sample_id,is_germline):
    ...
    if canonical_transcripts_file != None:
    ...
    elif mane_transcripts_file != None:
        can_df = pd.read_csv(mane_transcripts_file,sep='\t')
        can_df['Feature'] = can_df['Ensembl_nuc'].str.split('.',expand=True)[0]
        can_df['Hugo_Symbol'] = can_df['symbol']
        can_df = can_df[['Feature','Hugo_Symbol']]
