...
def process_mutect2(row, tum_col, nor_col):
    ...
    row['t_ref_reads'] = int(row[tum_col].split(':')[1].split(',')[0])
    row['t_alt_reads'] = int(row[tum_col].split(':')[1].split(',')[1])
    row['DP_tumor'] = int(row['t_ref_reads']+row['t_alt_reads'])
    row['n_ref_reads'] = int(row[nor_col].split(':')[1].split(',')[0])
    row['n_alt_reads'] = int(row[nor_col].split(':')[1].split(',')[1])
    row['DP_normal'] = int(row[nor_col].split(':')[3])
...
def cli(input_vcf, output_dir, tumor_sample_id, other_tumor_sample_id, panel_of_normals, normal_sample_id,germline):
    ...
    if normal_sample_id is not None:
      ...
      # WRITE RESULTS
      if germline == False:
          normal_sample_id = normal_sample_id.split('_',1)[1]
          tumor_sample_id = tumor_sample_id.split('_',1)[1]
